...
def _build_prompt(
    ...
    return (
        ...
        "must end with the citation number in brackets like [1] or [2].\n\n"
        ...
        "- Do NOT add a References or Sources section at the end — citations must be inline only.\n\n"
        "Example: \"**Audit Log Management** requires collecting logs from all assets [1]. "
        "Logs must be retained to prevent tampering [2].\"\n\n"
        f"{history_section}"
        f"Context:\n{context}\n\n"
        f"Question: {query}\n\n"
        "Answer (add [1], [2], etc. after each sentence):"
    )
